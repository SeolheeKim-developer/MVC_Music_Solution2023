...
namespace MVC_Music.Controllers
{
    ...
    public class InstrumentsController : CustomControllers.ElephantController
    {
        ...
        // GET/POST: Primary Musicians in Instrument/Notification/5
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (folksCount > 0)
                    {
                        ...
                        ViewData["Success"] = "Message sent to " + folksCount + " Musician"
                            + ((folksCount == 1) ? "." : "s.");
                    }
                    else
                    {
                        ViewData["Message"] = "Message NOT sent!  No Musicians in the Primary Instrument.";
                    }
                }
                catch (Exception ex)
                {
                    ...
                    ViewData["Message"] = " Could not send email message to the " + folksCount + " Musician"
                        + ((folksCount == 1) ? "" : "s") + $" in the Primary Instrument.Error:{errMsg}";
                }
            }
            return View();
        }
        ...
        public async Task<IActionResult> InsertFromExcel(IFormFile theExcel)
        {
            ...
            //give error message if user didn't upload the file and click the action button
            if (theExcel == null)
            {
            ...
            {
                alert = "Upload fails : cannot insert duplicate records";
            }
            TempData["Message"] = alert;
            return RedirectToAction(nameof(Index));
        }
